...
export function showProfileEditor() {
  ...
  if (overlay && body) {
    overlay.style.display = 'flex';
    // Аватар предпросмотр
    const avatarHtml = renderUserAvatar(userId, 'avatar-single');
    // Аксессуары чекбоксы
    const accessories = getConfig().ACCESSORY_ICONS;
    const accChecks = Object.keys(accessories).map(acc =>
      `<label><input type="checkbox" value="${sanitize(acc)}" ${user.accessories && user.accessories.includes(acc) ? 'checked' : ''}>${accessories[acc]}</label>`
    ).join(' ');
    body.innerHTML = `
      <div style="display:flex;align-items:center;gap:16px;justify-content:center;">${avatarHtml}</div>
      <div style="margin-top:10px;">
        <label>Имя: <input id="profileNameInput" value="${sanitize(user.name||'')}"></label><br>
        <span class="validation-message" id="profileNameHelp"></span>
        <label>Фото (URL): <input id="profilePhoto" value="${sanitize(user.photoURL||'')}"></label><br>
        <label>Цвет фона: <input type="color" id="profileBg" value="${sanitize(user.bgColor||'#4a90e2')}"></label>
        <label>Цвет текста: <input type="color" id="profileFg" value="${sanitize(user.textColor||'#fff')}"></label>
        <label>Цвет контура: <input type="color" id="profileOutline" value="${sanitize(user.outlineColor||'#00bfff')}"></label>
        <fieldset style="margin-top:10px;"><legend>Украшения</legend>${accChecks}</fieldset>
        ${state.userRole === 'admin' ? `<div><b>Роль:</b> ${sanitize(roleText)}</div>` : ''}
        <button id="saveProfileBtn" class="btn btn--save" style="margin-top:10px;">Сохранить</button>
      </div>
    `;
  }
}
